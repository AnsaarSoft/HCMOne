...
namespace ACHR.Screen
{
    class frm_Team:HRMSBaseForm
    {
        ...
        private void InitiallizeForm()
        {
            try
            {
                oDBDataTable = oForm.DataSources.DataTables.Add("Team");
                ...
                clNo = oColumn;
                oColumn.DataBind.Bind("Team", "No");

                oColumn = oColumns.Item("cl_ID");
                clID = oColumn;
                oColumn.DataBind.Bind("Team", "I");

                oColumn = oColumns.Item("cl_Name");
                clName = oColumn;
                oColumn.DataBind.Bind("Team", "N");

                oColumn = oColumns.Item("cl_Desc");
                clDesc = oColumn;
                oColumn.DataBind.Bind("Team", "D");
            }
            catch (Exception ex)
            {
        ...
        private void GetDataFromDataSource()
        {
            ...
            {
                oMat.Clear();
                dbHrPayroll.Refresh(System.Data.Linq.RefreshMode.OverwriteCurrentValues, dbHrPayroll.MstTeams);
                var Data = from n in dbHrPayroll.MstTeams select n;
        ...
        private void AddRowToDataBase(string Name, string Desc)
        {
            ...
            try
            {
                ...
                dbHrPayroll.MstTeams.InsertOnSubmit(Team);
                dbHrPayroll.SubmitChanges();
                try
                {
                    Query = string.Format("insert into OHTM (teamID ,Name ,descriptio ) values ({0},'{1}','{2}')", Team.Id, Team.Name, Team.Description);
                    RecSet.DoQuery(Query);
                }
                catch (Exception)
                {
                    oApplication.StatusBar.SetText("Code Already Exist in Table OHTM : Unable to add row in OHTM", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private void UpdateExistingRecord(string Name, string Desc, int ID)
        {
            ...
            try
            {
                //dbHrPayroll.ExecuteCommand("Update MstTeams set Name = {0},Description = {1},UpdateDate = {2} where ID = {3}", Name, Desc, DateTime.Now, ID);
                Team = (from v in dbHrPayroll.MstTeams where v.Id == ID select v).Single();
                Team.Name = Name;
                Team.Description = Desc;
                Team.UpdateDate = DateTime.Now;
                dbHrPayroll.SubmitChanges();
                Query = string.Format("Update OHTM set Name = '{0}',descriptio = '{1}'where teamID = {2}", Name, Desc, ID);
                RecSet.DoQuery(Query);
            }
            catch (Exception ex)
            {
        ...
        private bool CheckDuplicateRecords()
        {
            bool result = false;
            try
            {
                ...
                for (int i = 1; i <= oMat.RowCount; i++)
                {
                    ...
                    for (int j = 1; j <= oMat.RowCount; j++)
                    {
                        if (i == j)
                        { continue; }
                        else if (Name.ToLower().Equals((oMat.Columns.Item("cl_Name").Cells.Item(j).Specific as SAPbouiCOM.EditText).Value.ToLower()) ||
                           Desc.ToLower().Equals((oMat.Columns.Item("cl_Desc").Cells.Item(j).Specific as SAPbouiCOM.EditText).Value.ToLower()))
                        {
                            return (result = true);
                        }
                    }
