...
{
    class frm_TViewShiftSch : HRMSBaseForm
    {
        #region "Global Variable Area"

        SAPbouiCOM.Button btnSearch, btnCancel, btnSave;
                    ...
                    if (!string.IsNullOrEmpty(txEmpCode.Value))
                    {
                        ...
                        if (getEmp != null)
                        {
                            ...
                            if (EmpId > 0)
                            {
                                ...
                                {
                                    var AttendanceRegister = dbHrPayroll.TrnsTextileGroupAttendanceReg.Where(atr => atr.Date == x && atr.EmpID == EmpId).FirstOrDefault();
            ...
            if (getEmp != null)
            {
                try
                {
                    for (int i = 1; i < grdSceduledShift.RowCount + 1; i++)
                    {
                        ...
                        var DateRecord = dbHrPayroll.TrnsTextileGroupAttendanceReg.Where(a => a.EmpID == getEmp.ID && a.Date == dtDate).FirstOrDefault();
                        if (DateRecord != null)
                        {
                ...
                {
                    oApplication.StatusBar.SetText("Form: frm_TViewShiftSch Function: SaveRecords Msg: " + Ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);
                }
            }
        }
