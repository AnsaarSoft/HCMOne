...
{
    class frm_EmpNoLateAlw : HRMSBaseForm
    {
        #region Variables
        ...
        private void SaveRecord()
        {
            try
            {
                ...
                int chkCount = (from a in dbHrPayroll.TrnsEmployeeNoLateAllowance where a.DocNum.ToString() == DocNum select a).Count();
                if (chkCount > 0)
                {
                    var oDoc = (from a in dbHrPayroll.TrnsEmployeeNoLateAllowance where a.DocNum.ToString() == DocNum select a).FirstOrDefault();
                    if (oDoc != null)
                    {
                        ...
                        for (int i = 0; i < dtMain.Rows.Count; i++)
                        {
                            ...
                            {
                                var oLine = (from a in dbHrPayroll.TrnsEmployeeNoLateAllowanceDetail
                                             where a.TrnsEmployeeNoLateAllowance.DocNum.ToString() == DocNum && a.MstEmployee.EmpID == empcode
                                ...
                                {
                                    oLine = new TrnsEmployeeNoLateAllowanceDetail();
                                    oDoc.TrnsEmployeeNoLateAllowanceDetail.Add(oLine);
                ...
                {
                    var oDoc = new TrnsEmployeeNoLateAllowance();
                    dbHrPayroll.TrnsEmployeeNoLateAllowance.InsertOnSubmit(oDoc);
                    ...
                    for (int i = 0; i < dtMain.Rows.Count; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(empcode))
                        {
                            ...
                            var oLine = new TrnsEmployeeNoLateAllowanceDetail();
                            oDoc.TrnsEmployeeNoLateAllowanceDetail.Add(oLine);
                            oLine.EmpID = oEmp.ID;
                            oLine.CreatedBy = oCompany.UserName;
                            oLine.UpdatedBy = oCompany.UserName;
        ...
        private void ClearRecord()
        {
            ...
            {
                string docnum = "";
                var oValue = (from a in dbHrPayroll.TrnsEmployeeNoLateAllowance select a).Max(a => a.DocNum).GetValueOrDefault();
                if (oValue != null)
                {
        ...
        private void FillRecord()
        {
            try
            {
                ...
                {
                    var oDoc = (from a in dbHrPayroll.TrnsEmployeeNoLateAllowance
                    ...
                    txtFilePath.Value = "";
                    int i = 0;
                    if (oDoc.TrnsEmployeeNoLateAllowanceDetail.Count > 0) dtMain.Rows.Clear();
                    foreach (var oLine in oDoc.TrnsEmployeeNoLateAllowanceDetail)
                    {
                        dtMain.Rows.Add(1);
        ...
        private void FillElementForCalculation(SAPbouiCOM.ComboBox pCombo)
        {
            ...
            {
                oApplication.StatusBar.SetText("FillElementForCalculation : " + ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);
            }
        }
        ...
        private void GetData()
        {
            ...
            {
                CodeIndex.Clear();
                var oDocuments = (from a in dbHrPayroll.TrnsEmployeeNoLateAllowance select a).ToList();
                Int32 i = 0;
