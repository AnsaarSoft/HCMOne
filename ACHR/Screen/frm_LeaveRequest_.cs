...
namespace ACHR.Screen
{
    ...
    {    
       ...
       // private SAPbouiCOM.Application oApplication;
        ...
        SAPbouiCOM.EditText txtDocNum, txtEmpCode, txtEmpName, txtFromDate, txtToDate, txtTotal, txtApprocalStatus, txtDocStatus, txtBF, txtEntitled, txtTotalAvb, txtTotalAccured, txtUsed, txtRequeted, txtApproved, txtBalance;
                ...
                if (strUnit.ToUpper() == "DAY" || strUnit.ToUpper() == "HALFDAY")
                {
                    if (!string.IsNullOrEmpty(txtFromDate.Value) && !string.IsNullOrEmpty(txtToDate.Value))
                    {
                        ...
                        for (DateTime x = dtFrom; x <= dtTo; x = x.AddDays(1))
                        {
                            ...
                            else if (ShiftDay != null && ShiftDayTS == null)
                            {
        ...
        private void AddLeaveRequest()
        {
            ...
            try
            {
                ...
                //dbHrPayroll.TrnsLeavesRequest.Count() + 1;
        ...
        private Boolean ValidationAccured()
        {
            try
            {
                ...
                //{
                    ...
                    var CurrentCalanderPeriod = (from a in dbHrPayroll.CfgPeriodDates 
                                                 where a.CalCode == Convert.ToString(CurrentCalander.Code) 
                                                 //&& a.StartDate >= ToDate 
                                                 select a).FirstOrDefault();
        ...
        private Boolean AddValidation()
        {
            try
            {
                ...
                var oEmp = (from a in dbHrPayroll.MstEmployee 
                            ...
                            select a).FirstOrDefault();
                var LockedPeriod = (from a in dbHrPayroll.CfgPeriodDates 
                                    where a.PayrollId == oEmp.PayrollID  
                                    && a.StartDate <= StartDate 
                                    && a.EndDate >= StartDate 
                                    && a.FlgLocked == true 
                                    select a).FirstOrDefault();
                var CurrentPeriod = (from a in dbHrPayroll.CfgPeriodDates 
                                     where a.PayrollId == oEmp.PayrollID 
                                     && a.StartDate <= StartDate 
                                     && a.EndDate >= StartDate 
                                     && a.FlgLocked == false 
                                     select a).FirstOrDefault();
                var CurrentPeriod2 = (from a in dbHrPayroll.CfgPeriodDates 
                                      where a.PayrollId == oEmp.PayrollID 
                                      && a.StartDate <= EndDate 
                                      && a.EndDate >= EndDate 
                                      && a.FlgLocked == false 
        ...
        private void FillUnitsValues()
        {
            try
            {
                var oEmployee = (from a in dbHrPayroll.MstEmployee where a.EmpID == txtEmpCode.Value.Trim() select a).FirstOrDefault();
                if (oEmployee == null) return;
                int HourInPayroll = 0, UnitsInOneDay = 0;
                HourInPayroll = Convert.ToInt32(oEmployee.CfgPayrollDefination.WorkHours);
                if (cbUnits.Value.Trim().ToUpper() == "DAY")
                {
                    UnitsInOneDay = (HourInPayroll * 60) / UnitValue;
                }
                if (cbUnits.Value.Trim().ToUpper() == "HALFDAY")
                {
                    UnitsInOneDay = ((HourInPayroll * 60) / UnitValue) / 2;
                }
                if (cbUnits.Value.Trim().ToUpper() == "MIN")
                {
                    UnitsInOneDay = (HourInPayroll * 60) / UnitValue;
                }
                if (cbUnits.Value.Trim().ToUpper() == "HOUR")
                {
                    UnitsInOneDay = HourInPayroll / UnitValue;
                }
