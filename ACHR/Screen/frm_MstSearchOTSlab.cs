...
{
    class frm_MstSearchOTSlab : HRMSBaseForm
    {
        #region Variables
        ...
        private void GetData()
        {
            try
            {
                var oCollection = (from a in dbHrPayroll.TrnsOTSlab select a).ToList().OrderBy(a => a.CreateDt);
                if (oCollection.Count() > 0)
                {
                    ...
                    foreach (var Line in oCollection)
                    {
                        ...
                        dtMain.SetValue(code.DataBind.Alias, i, Line.SlabCode);
                        dtMain.SetValue(createdate.DataBind.Alias, i, Line.CreateDt);
                        i++;
                    }
