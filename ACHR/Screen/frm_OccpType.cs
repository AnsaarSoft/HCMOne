...
{
    class frm_OccpType:HRMSBaseForm 
    {
        ...
        private MstOccupationType OccpType;
        ...
        private void InitiallizeForm()
        {
            try
            {
                oDBDataTable = oForm.DataSources.DataTables.Add("OccpType");
                ...
                clNo = oColumn;
                oColumn.DataBind.Bind("OccpType", "No");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_ID");
                clID = oColumn;
                oColumn.DataBind.Bind("OccpType", "I");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Code");
                clCode = oColumn;
                oColumn.DataBind.Bind("OccpType", "C");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Desc");
                clDesc = oColumn;
                oColumn.DataBind.Bind("OccpType", "D");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Active");
                clActive = oColumn;
                oColumn.DataBind.Bind("OccpType", "A");
                oColumn.TitleObject.Sortable = false;
            }
            catch (Exception ex)
            {
        ...
        private void GetDataFromDataSource()
        {
            ...
            {
                oMat.Clear();
                dbHrPayroll.Refresh(System.Data.Linq.RefreshMode.OverwriteCurrentValues, dbHrPayroll.MstOccupationType);
                var Data = from n in dbHrPayroll.MstOccupationType select n;
                oDBDataTable.Rows.Clear();
                int i = 0;
                foreach (var v in Data)
                {
                    ...
                    oDBDataTable.SetValue("A", i, v.StatusOcupation == true ? "Y" : "N");
                    i += 1;
                }
        ...
        private void AddRowToDataBase(string Code, string Desc, string Active)
        {
            try
            {
                OccpType = new MstOccupationType();
                OccpType.Code = Code;
                OccpType.Description = Desc;
                OccpType.StatusOcupation = (Active == "Y" ? true : false);
                OccpType.CreateDate = DateTime.Now;
                OccpType.UserId = oCompany.UserSignature.ToString();
                dbHrPayroll.MstOccupationType.InsertOnSubmit(OccpType);
        ...
        private void UpdateExistingRecord(string Code, string Desc, bool Active, int ID)
        {
            try
            {
                OccpType = (from v in dbHrPayroll.MstOccupationType where v.Id == ID select v).Single();
                OccpType.Code = Code;
                OccpType.Description = Desc;
                OccpType.StatusOcupation = Active;
                OccpType.UpdateDate = DateTime.Now;
                OccpType.UserId = oCompany.UserSignature.ToString();
                dbHrPayroll.SubmitChanges();
            }
            catch (Exception ex)
        ...
        private void DeleteRecord(int Row)
        {
            try
            {
                ...
                MstOccupationType Record = (from v in dbHrPayroll.MstOccupationType where v.Id == ID select v).Single();
                dbHrPayroll.MstOccupationType.DeleteOnSubmit(Record);
                dbHrPayroll.SubmitChanges();
