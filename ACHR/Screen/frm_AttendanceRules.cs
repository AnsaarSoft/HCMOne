...
{
    class frm_AttendanceRules : HRMSBaseForm
    {
        ...
        public override void CreateForm(SAPbouiCOM.Application SboApp, string strXml, SAPbobsCOM.Company cmp, string frmId)
        {
            ...
            {
                oApplication.StatusBar.SetText("Form: Frm_AttendanceRules Function: CreateForm Msg: " + Ex.Message, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);
            }
        }
            ...
            try
            {
                ...
                if (AttRules != null && AttRules.Count > 0)
                {
                    ...
                    foreach (var Rule in AttRules)
                    {
                        ...
                        {
                            var LeaveTypeID = dbHrPayroll.MstLeaveType.Where(lt => lt.ID == Rule.LeaveType).FirstOrDefault().Code;
                            dtRules.SetValue("LeaveType", i, LeaveTypeID);
                        }
                        else
                        {
