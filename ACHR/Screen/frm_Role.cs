...
namespace ACHR.Screen
{
    class frm_Role:HRMSBaseForm
    {
        ...
        private void InitiallizeForm()
        {
            try
            {
                oDBDataTable = oForm.DataSources.DataTables.Add("Roles");
                ...
                clNo = oColumn;
                oColumn.DataBind.Bind("Roles", "No");

                oColumn = oColumns.Item("cl_ID");
                clID = oColumn;
                oColumn.DataBind.Bind("Roles", "I");

                oColumn = oColumns.Item("cl_Name");
                clName = oColumn;
                oColumn.DataBind.Bind("Roles", "N");

                oColumn = oColumns.Item("cl_Desc");
                clDesc = oColumn;
                oColumn.DataBind.Bind("Roles", "D");
            }
            catch (Exception ex)
            {
        ...
        private void GetDataFromDataSource()
        {
            ...
            {
                oMat.Clear();
                dbHrPayroll.Refresh(System.Data.Linq.RefreshMode.OverwriteCurrentValues, dbHrPayroll.MstRoles);
                var Data = from n in dbHrPayroll.MstRoles select n;
        ...
        private void AddRowToDataBase(string Name, string Desc)
        {
            ...
            try
            {
                ...
                dbHrPayroll.MstRoles.InsertOnSubmit(Role);
                dbHrPayroll.SubmitChanges();
                try
                {
                    Query = string.Format("insert into OHTY (typeID ,Name ,descriptio ) values ({0},'{1}','{2}')", Role.Id, Role.Name, Role.Description);
                    RecSet.DoQuery(Query);
                }
                catch (Exception)
                {
                    oApplication.StatusBar.SetText("Code Already Exist in Table OHTY : Unable to add row in OHTY", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private void UpdateExistingRecord(string Name, string Desc, int ID)
        {
            ...
            try
            {
                //dbHrPayroll.ExecuteCommand("Update MstRoles set Name = {0},Description = {1},UpdateDate = {2} where ID = {3}", Name, Desc, DateTime.Now, ID);
                Role = (from v in dbHrPayroll.MstRoles where v.Id == ID select v).Single();
                Role.Name = Name;
                Role.Description = Desc;
                Role.UpdateDate = DateTime.Now;
                dbHrPayroll.SubmitChanges();
                Query = string.Format("Update OHTY set Name = '{0}',descriptio = '{1}'where typeID = {2}", Name, Desc, ID);
                RecSet.DoQuery(Query);
            }
            catch (Exception ex)
            {
