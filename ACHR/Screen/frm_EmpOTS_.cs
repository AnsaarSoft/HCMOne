...
namespace ACHR.Screen
{
    class frm_EmpOTS : HRMSBaseForm
    {
        ...
        private void FillPeriod1(string payroll)
        {
            try
            {
                //dtOT.Rows.Clear();
        ...
        private void FillPeriodold(string payroll)
        {
            try
            {
                ...
                {
                    CfgPayrollDefination pr = (from p in dbHrPayroll.CfgPayrollDefination where p.ID.ToString() == payroll.Trim() select p).FirstOrDefault();
                    IEnumerable<CfgPeriodDates> perioddateList = pr.CfgPeriodDates.Where(d => d.FlgLocked == false).ToList();
                    ...
                    try
                    {
                        ...
                        {
                            cmbPeriod.Select(selId, SAPbouiCOM.BoSearchKey.psk_ByValue);
                        }
                        else
                        {
            ...
            {
                oApplication.StatusBar.SetText("Error in Function FillPeriod.Error is  " + ex.Message + " Trace: " + ex.StackTrace, SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);
            }
        }
        ...
        private decimal setRowAmnt(MstEmployee emp, decimal Overtimehours)
        {
            ...
            try
            {
                ...
                decimal BaseAmount = baseValue;  //Convert.ToDecimal(dtOT.GetValue("BaseVal", rowNum));
                decimal Val = value; //Convert.ToDecimal(dtOT.GetValue("Value", rowNum));
                if (PerMonthCap > 0)
                {
                    if (hours > PerMonthCap)
                    {
                        hours = PerMonthCap;
                        MsgWarning("Per Month Cap was hit during OT calculations");
                    }
                }
                if (flgFormula)
                {
                ...
                {
                    BaseAmount = BaseAmount + fixValueCola;
                    BaseAmount = BaseAmount * 12;
                    BaseAmount = BaseAmount / daysinYear;
                    BaseAmount = BaseAmount / workhours;
                    ...
                    amount = ((BaseAmount * Val / 100) + baseAmountFormula) * hours;
                ...
                else
                {
                    ...
                    {
                        //amount = ((baseAmoun / monthHours) * Val / 100) * hours;
                        amount = ((BaseAmount / monthHours) * Val / 100) * hours;
                    }
                }
                    ...
                    foreach (string OneComponent in oElementList)
                    {
                        ...
                        else
                        {
                            ...
                            TrnsEmployeeElementDetail oEle = (from a in dbHrPayroll.TrnsEmployeeElementDetail
                                                              ...
                                                              //&& a.FlgActive.GetValueOrDefault() == true
                                                              //&& a.FlgOneTimeConsumed.GetValueOrDefault() == false
