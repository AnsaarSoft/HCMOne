...
namespace ACHR.Screen
{
    class frm_Certif:HRMSBaseForm
    {
        ...
        private MstCertification Certification;
        ...
        private void InitiallizeForm()
        {
            try
            {
                oDBDataTable = oForm.DataSources.DataTables.Add("Certification");
                ...
                clNo = oColumn;
                oColumn.DataBind.Bind("Certification", "No");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_ID");
                clID = oColumn;
                oColumn.DataBind.Bind("Certification", "I");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Name");
                clName = oColumn;
                oColumn.DataBind.Bind("Certification", "N");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Desc");
                clDesc = oColumn;
                oColumn.DataBind.Bind("Certification", "D");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("clactive");
                clActive = oColumn;
                oColumn.DataBind.Bind("Certification", "Active");
                oColumn.TitleObject.Sortable = false;
            }
            catch (Exception ex)
            {
        ...
        private void GetDataFromDataSource()
        {
            ...
            {
                oMat.Clear();
                dbHrPayroll.Refresh(System.Data.Linq.RefreshMode.OverwriteCurrentValues, dbHrPayroll.MstCertification);
                var Data = from n in dbHrPayroll.MstCertification select n;
        ...
        private void AddRowToDataBase(string Name, string Desc, bool Active)
        {
            try
            {
                Certification = new MstCertification();
                Certification.Name = Name;
                Certification.Description = Desc;
                Certification.FlgActive = Active;
                Certification.CreateDate = DateTime.Now;
                Certification.UserId = oCompany.UserName;
                dbHrPayroll.MstCertification.InsertOnSubmit(Certification);
        ...
        private void UpdateExistingRecord(string Name, string Desc, int ID, bool Active)
        {
            try
            {
                ...
                Certification = (from v in dbHrPayroll.MstCertification where v.Id == ID select v).Single();
                Certification.Name = Name;
                Certification.Description = Desc;
                Certification.FlgActive = Active;
                Certification.UpdateDate = DateTime.Now;
                Certification.UpdatedBy = oCompany.UserName;
                dbHrPayroll.SubmitChanges();
            }
            catch (Exception ex)
            {
