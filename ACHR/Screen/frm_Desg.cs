    ...
    class frm_Desg:HRMSBaseForm
    {
        ...
        private void InitiallizeForm()
        {
            try
            {
                ...
                oDBDataTable = oForm.DataSources.DataTables.Add("Designations");
                ...
                clNo = oColumn;
                oColumn.DataBind.Bind("Designations", "No");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_ID");
                clID = oColumn;
                oColumn.DataBind.Bind("Designations", "I");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Name");
                clName = oColumn;
                oColumn.DataBind.Bind("Designations", "N");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Desc");
                clDesc = oColumn;
                oColumn.DataBind.Bind("Designations", "D");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("clactive");
                clActive = oColumn;
                oColumn.DataBind.Bind("Designations", "Active");
                oColumn.TitleObject.Sortable = false;
        ...
        private void GetDataFromDataSource()
        {
            try
            {
                ...
                var Data = from n in dbHrPayroll.MstDesignation select n;
        ...
        public override void etFormBeforeResize(ref ItemEvent pVal, ref bool BubbleEvent)
        {
            base.etFormBeforeResize(ref pVal, ref BubbleEvent);
        ...
        private void AddRowToDataBase(string Name, string Desc, bool Status)
        {
            try
            {
                Designation = new MstDesignation();
                Designation.Name = Name;
                Designation.Description = Desc;
                Designation.FlgActive = Status;
        ...
        private void UpdateExistingRecord(string Name, string Desc, int ID, bool Status)
        {
            try
            {
                //dbHrPayroll.ExecuteCommand("Update MstDesignation set Name = {0},Description = {1},UpdateDate = {2} where ID = {3}", Name, Desc, DateTime.Now, ID);
                Designation = (from v in dbHrPayroll.MstDesignation where v.Id == ID select v).Single();
                Designation.Name = Name;
                Designation.Description = Desc;
                Designation.FlgActive = Status;
