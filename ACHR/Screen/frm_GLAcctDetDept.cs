...
{
    class frm_GLAcctDetDept : HRMSBaseForm
    {
        ...
        SAPbouiCOM.EditText txBasicE, txArrE, txLevE, txEOSE, txGrtA, txITE, txBasicP, txArrP, txLvP, txEOSP, txGrtP, txITP, txDff, GLDID, txDept;
        SAPbouiCOM.Item ImtAdv, IcoAdvCode, IcoAdvDesc, IcoAdvAct, ItxBasicE, ItxArrE, ItxLevE, ItxEOSE, ItxGrtA, ItxITE, ItxBasicP, ItxArrP, ItxLvP, ItxEOSP, ItxGrtP, ItxITP, ItxDff, IGLDID, ItxDept;
        //**********************************
        ...
        private void InitiallizeForm()
        {
            ...
            oForm.DataSources.UserDataSources.Add("txDept", SAPbouiCOM.BoDataType.dt_SHORT_TEXT, 50); // Days of Month
            txDept = oForm.Items.Item("txDept").Specific;
            ItxDept = oForm.Items.Item("txDept");
            txDept.DataBind.SetBound(true, "", "txDept");
        ...
        {
            SearchKeyVal.Clear();
            string strSql = sqlString.getSql("glDept", SearchKeyVal);
            ...
            if (st.Rows.Count > 0)
            {
                ...
                txDept.Value  = st.Rows[0][1].ToString();
                _fillFields();
            }
        }
        ...
        private void inicontrolls()
        {
            ...
            {
                dtBonus.SetValue("CostAcct", i, "");
                dtBonus.SetValue("BalAcct", i, "");
                dtBonus.SetValue("CostDesc", i, "");
                dtBonus.SetValue("BalDesc", i, "");
            }
            mtBonus.LoadFromDataSource();
        }
        public void _fillFields()
        {
            if (txDept.Value.ToString().Trim() == "") return;
            oForm.Freeze(true);
            MstGLDetermination gl;
            MstDepartment dept = (from p in dbHrPayroll.MstDepartment where p.Code.ToString() == txDept.Value.ToString() select p).Single();
            SAPbouiCOM.StaticText lType =  oForm.Items.Item("lType").Specific;
            lType.Caption = dept.DeptName;
           
            inicontrolls();
            int cnt = (from p in dbHrPayroll.MstGLDetermination where p.GLType == "DEPT" && p.GLValue.ToString() == dept.ID.ToString() select p).Count();
            if (cnt == 0)
            {
                inicontrolls();
            }
            ...
            {
                SelId = dept.ID;
                gl = (from p in dbHrPayroll.MstGLDetermination where p.GLType == "DEPT" && p.GLValue.ToString() == dept.ID.ToString() select p).Single();
        ...
        private void updateDbWithMat()
        {
            ...
            int cnt = (from p in dbHrPayroll.MstGLDetermination where p.GLType == "DEPT" && p.GLValue == SelId  select p).Count();

            if (cnt > 0)
            {
                gl = (from p in dbHrPayroll.MstGLDetermination where p.GLType == "DEPT" && p.GLValue == SelId select p).Single();
            }
            else
            {
                gl = new MstGLDetermination();
