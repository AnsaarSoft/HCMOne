...
{
    class frm_VAttProcess : HRMSBaseForm
    {
        ...
        private void LoadEmployeeAttendanceRecordOrderByDate()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(txtFromDate.Value) && !string.IsNullOrEmpty(txtToDate.Value))
                {
                    ...
                    //EmployeeWorkingHourDateWise obj = GetEmpHours("0000001642", startDate, EndDate);
                            ...
                            if (intEmpID > 0)
                            {
                                ...
                                else if (AttendanceRegister != null && AttendanceRegister.FlgProcessed != true)
                                {
                                    ...
                                    if (!InflgOverlap && !OutflgOverlap)
                                    {
                                        ...
                                        var TempAttendance = dbHrPayroll.TrnsTempAttendance.Where(tr => tr.EmpID == EmpREcord.EmpID && tr.PunchedDate.Value.Date == x).ToList();
                                        if (TempAttendance != null && TempAttendance.Count > 0)
                                        {
                                            strTimeIn = Convert.ToString(TempAttendance.Where(str => str.In_Out == "0").Min(y => y.PunchedTime)) == null ? "" : Convert.ToString(TempAttendance.Where(str =>  str.In_Out == "0").Min(y => y.PunchedTime));
                                            strTimeOut = Convert.ToString(TempAttendance.Where(str => str.In_Out == "1").Max(y => y.PunchedTime)) == null ? "" : Convert.ToString(TempAttendance.Where(str =>  str.In_Out == "1").Max(y => y.PunchedTime));
                                            //EmpREcord
                                            EmployeeWorkingHourDateWise obj = GetEmpHours(EmpREcord.EmpID, x, x);
                                            try
                                            {
                                                String[] T = obj.list[0].workingHors.ToString().Split(':');
                                                strWorkHours = T[0]  + ":" + T[1];
                                            }
                                            catch
                                            {
                                                strWorkHours = "00:00";
                                        ...
                                        {
                                            strWorkHours = CalculateOverTimeHoursInandOutTime(strWorkHours, "01:00");
                                        }
                                    }                                    
                                    else if (!InflgOverlap && OutflgOverlap)
                                    {
                                        ...
                                        {
                                            strTimeOut = Convert.ToString(TempAttendanceOverlap.Where(str =>  str.In_Out == "1").Max(y => y.PunchedTime)) == null ? "" : Convert.ToString(TempAttendanceOverlap.Where(str =>  str.In_Out == "1").Max(y => y.PunchedTime));
                                        }
                                        ...
                                        EmployeeWorkingHourDateWise objdtx = GetEmpHours(EmpREcord.EmpID, dtx, dtx);
                                        string strwrkHrSec = "00:00";
                                        try
                                        {
                                            String[] T = objdtx.list[0].workingHors.ToString().Split(':');
                                            strwrkHrSec = T[0] + ":" + T[1];
                                        }
                                        catch
                                        {
                                        ...
                                        if (!string.IsNullOrEmpty(strWorkHours) && strWorkHours != "00:00")
                                        {
                                            strWorkHours = CalculateOverTimeHoursInandOutTime(strWorkHours, strwrkHrSec);
                                            strWorkHours = CalculateOverTimeHoursInandOutTime(strWorkHours, "01:00");
                                        }
                                    }
                                    else if (InflgOverlap && OutflgOverlap)
                                    {
                                        ...
                                        if (TempAttendance != null && TempAttendance.Count > 0)
                                        {
                                            ...
                                            strWorkHours = CalculateWorkHours(strTimeIn, strTimeOut);
                                            //EmpREcord
                                            EmployeeWorkingHourDateWise obj = GetEmpHours(EmpREcord.EmpID, dtx, dtx);
                                            try
                                            {
                                                String[] T = obj.list[0].workingHors.ToString().Split(':');
                                                strWorkHours = T[0] + ":" + T[1];
                                        ...
                                        if (AttendanceRegister.FlgOffDay.Value != true)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if (!string.IsNullOrEmpty(TempLeaveHours) && TempLeaveHours != "00:00")
                                                {
                                                    ...
                                                    LeaveCount = GetOTHrsRatio(shiftHours, TempLeaveHours);
        ...
        //Code BY Azeem
        private EmployeeWorkingHourDateWise GetEmpHours(String Empid1, DateTime startdt, DateTime enddt)
        {
            EmployeeWorkingHourDateWise Parent = new EmployeeWorkingHourDateWise();
            List<TrnsTempAttendance> list;
            try
            {
                list = (from a in dbHrPayroll.TrnsTempAttendance where a.EmpID == Empid1 && a.PunchedDate.Value.Date >= startdt && a.PunchedDate.Value.Date <= enddt orderby a.ID select a).ToList();

                List<String> combinationlist = new List<String>();
                combinationlist.Add("0");//In
                combinationlist.Add("1");//Out
                String CombinationIndex = combinationlist[0];

                if (list != null)
                {
                    for (int i = 0; i < list.Count; i++)
                    {
                        if (combinationlist[1].Equals(list[i].In_Out.ToString()) || combinationlist[1].Equals("0" + list[i].In_Out.ToString()))
                        {
                            // on Finding first out.. 1 index back if i will find In then its oky else delete
                            if (i - 1 >= 0 && list[i - 1].In_Out.ToString().Equals(combinationlist[0]))
                            {
                                list[i].CostCenter = "88";
                                list[i - 1].CostCenter = "88";
                            }

                        }

                    }
                    list.RemoveAll(emp => emp.CostCenter != "88");

                    var list_distinctDate = list.Select(x => x.PunchedDate).Distinct().ToList();

                    for (int l = 0; l < list_distinctDate.Count; l++)
                    {
                        List<TrnsTempAttendance> obj_list1 = (from a in list where a.PunchedDate == list_distinctDate[l] orderby a.ID select a).ToList();

                        TimeSpan WorkingTimePerDay = new TimeSpan(0, 0, 0);

                        if (obj_list1.Count % 2 == 0)//mean All In Out interval complete on 1 date
                        {
                            for (int j = 0; j < obj_list1.Count; j++)
                            {
                                //hours
                                if (j % 2 != 0)//skip first n get second in other words 1 (IN,OUT)
                                {
                                    try
                                    {
                                        TimeSpan T1 = new TimeSpan(Convert.ToInt32(obj_list1[j].PunchedTime.Split(':')[0]), Convert.ToInt32(obj_list1[j].PunchedTime.Split(':')[1]), 0);
                                        TimeSpan T2 = new TimeSpan(Convert.ToInt32(obj_list1[j - 1].PunchedTime.Split(':')[0]), Convert.ToInt32(obj_list1[j - 1].PunchedTime.Split(':')[1]), 0);
                                        TimeSpan TimeDiff = T1 - T2;
                                        WorkingTimePerDay = WorkingTimePerDay + TimeDiff;
                                    }
                                    catch
                                    {
                            ...
                            Parent.list.Add(new EmployeeDateHours(list_distinctDate[l].Value, WorkingTimePerDay));
                        }
                        else////How to handle last Inveral which lay on two days
                        {
                            if (obj_list1[0].In_Out == combinationlist[1])//[OUT]
                            {
                                WorkingTimePerDay = WorkingTimePerDay + new TimeSpan(Convert.ToInt32(obj_list1[0].PunchedTime.Split(':')[0]), Convert.ToInt32(obj_list1[0].PunchedTime.Split(':')[1]), 0) - new TimeSpan(0, 0, 0);
                                for (int j = 1; j < obj_list1.Count; j++)
                                {
                                    //hours
                                    if (j % 2 != 0)//skip first n get second in other words 1 (IN,OUT)
                                    {
                            ...
                            {
                                for (int j2 = 0; j2 < obj_list1.Count - 1; j2++)
                                {
                                    //hours
                                    if (j2 % 2 != 0)//skip first n get second in other words 1 (IN,OUT)
                                    {
                                        try
                                        {
                                            TimeSpan T1 = new TimeSpan(Convert.ToInt32(obj_list1[j2].PunchedTime.Split(':')[0]), Convert.ToInt32(obj_list1[j2].PunchedTime.Split(':')[1]), 0);
                                            TimeSpan T2 = new TimeSpan(Convert.ToInt32(obj_list1[j2 - 1].PunchedTime.Split(':')[0]), Convert.ToInt32(obj_list1[j2 - 1].PunchedTime.Split(':')[1]), 0);
                                            TimeSpan TimeDiff = T1 - T2;
                                            WorkingTimePerDay = WorkingTimePerDay + TimeDiff;
                                        }
                                        catch
                                ...
                                TimeSpan temp = new TimeSpan(23, 59, 0) - new TimeSpan(Convert.ToInt32(obj_list1[obj_list1.Count - 1].PunchedTime.Split(':')[0]), Convert.ToInt32(obj_list1[obj_list1.Count - 1].PunchedTime.Split(':')[1]), 0);
                                WorkingTimePerDay = WorkingTimePerDay + temp;
                                Parent.list.Add(new EmployeeDateHours(list_distinctDate[l].Value, WorkingTimePerDay));
                            }
        ...
        public class EmployeeWorkingHourDateWise
        {
            public String Empid = "";
            public List<EmployeeDateHours> list = new List<EmployeeDateHours>();

        }
        public class EmployeeDateHours
        {
            public DateTime _Date;
            public TimeSpan workingHors;
            public EmployeeDateHours(DateTime dt, TimeSpan workingHors)
            {
                _Date = dt;
        ...
        private string GetTotalEmpWrkHrs(string strEmpid, DateTime dtPunchDate)
        {
            try
            {
                string strTotalWrkInHours = "";
                var TempAttendance = dbHrPayroll.TrnsTempAttendance.Where(tr => tr.EmpID == strEmpid && tr.PunchedDate.Value.Date == dtPunchDate).ToList();
                if (TempAttendance != null)
                {
                    TempAttendance = TempAttendance.OrderBy(d => d.PunchedTime).ToList();
                    foreach (TrnsTempAttendance item in TempAttendance)
                    {
                        string PunchTime = item.PunchedTime;
                    }
                }
                return strTotalWrkInHours;
