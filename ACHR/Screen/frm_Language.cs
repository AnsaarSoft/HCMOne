...
{
    class frm_Language:HRMSBaseForm
    {
        ...
        private void InitiallizeForm()
        {
            try
            {
                ...
                oDBDataTable = oForm.DataSources.DataTables.Add("Language");
                ...
                clNo = oColumn;
                oColumn.DataBind.Bind("Language", "No");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_ID");
                clID = oColumn;
                oColumn.DataBind.Bind("Language", "I");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Name");
                clName = oColumn;
                oColumn.DataBind.Bind("Language", "N");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Desc");
                clDesc = oColumn;
                oColumn.DataBind.Bind("Language", "D");
                oColumn.TitleObject.Sortable = false;
            }
            catch (Exception ex)
            {
        ...
        private void GetDataFromDataSource()
        {
            ...
            {
                oMat.Clear();
                dbHrPayroll.Refresh(System.Data.Linq.RefreshMode.OverwriteCurrentValues, dbHrPayroll.MstLanguages);
                var Data = from n in dbHrPayroll.MstLanguages select n;
        ...
        private void AddRowToDataBase(string Name, string Desc)
        {
            try
            {
                Language = new MstLanguages();
                Language.Name = Name;
                Language.Description = Desc;
                Language.CreateDate = DateTime.Now;
                dbHrPayroll.MstLanguages.InsertOnSubmit(Language);
        ...
        private void UpdateExistingRecord(string Name, string Desc, int ID)
        {
            try
            {
                //dbHrPayroll.ExecuteCommand("Update MstLanguages set Name = {0},Description = {1},UpdateDate = {2} where ID = {3}", Name, Desc, DateTime.Now, ID);
                Language = (from v in dbHrPayroll.MstLanguages where v.Id == ID select v).Single();
                Language.Name = Name;
                Language.Description = Desc;
