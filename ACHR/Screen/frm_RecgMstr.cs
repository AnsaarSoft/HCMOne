...
{
    class frm_RecgMstr:HRMSBaseForm 
    {
        ...
        private MstRecognition Recognition;
        ...
        private void InitiallizeForm()
        {
            try
            {
                oDBDataTable = oForm.DataSources.DataTables.Add("Recognition");
                ...
                clNo = oColumn;
                oColumn.DataBind.Bind("Recognition", "No");

                oColumn = oColumns.Item("cl_ID");
                clID = oColumn;
                oColumn.DataBind.Bind("Recognition", "I");

                oColumn = oColumns.Item("cl_Code");
                clCode = oColumn;
                oColumn.DataBind.Bind("Recognition", "C");

                oColumn = oColumns.Item("cl_Desc");
                clDesc = oColumn;
                oColumn.DataBind.Bind("Recognition", "D");

                oColumn = oColumns.Item("cl_Active");
                clActive = oColumn;
                oColumn.DataBind.Bind("Recognition", "A");
            }
            catch (Exception ex)
            {
        ...
        private void GetDataFromDataSource()
        {
            ...
            {
                oMat.Clear();
                dbHrPayroll.Refresh(System.Data.Linq.RefreshMode.OverwriteCurrentValues, dbHrPayroll.MstRecognition);
                var Data = from n in dbHrPayroll.MstRecognition select n;
                oDBDataTable.Rows.Clear();
                int i = 0;
                foreach (var v in Data)
                {
                    ...
                    oDBDataTable.SetValue("I", i, (int)v.Id);
                    oDBDataTable.SetValue("C", i, v.Code);
                    oDBDataTable.SetValue("D", i, v.Description);
                    oDBDataTable.SetValue("A", i, v.StatusRecognition == true ? "Y" : "N");
                    i += 1;
                }
        ...
        private void AddRowToDataBase(string Code, string Desc, string Active)
        {
            try
            {
                Recognition = new MstRecognition();
                Recognition.Code = Code;
                Recognition.Description = Desc;
                Recognition.StatusRecognition = (Active == "Y" ? true : false);
                Recognition.CreateDate = DateTime.Now;
                Recognition.UserId = oCompany.UserSignature.ToString();
                dbHrPayroll.MstRecognition.InsertOnSubmit(Recognition);
        ...
        private void UpdateExistingRecord(string Code, string Desc, bool Active, int ID)
        {
            try
            {
                Recognition = (from v in dbHrPayroll.MstRecognition where v.Id == ID select v).Single();
                Recognition.Code = Code;
                Recognition.Description = Desc;
                Recognition.StatusRecognition = Active;
                Recognition.UpdateDate = DateTime.Now;
                Recognition.UserId = oCompany.UserSignature.ToString();
                dbHrPayroll.SubmitChanges();
            }
            catch (Exception ex)
        ...
        private void DeleteRecord(int Row)
        {
            try
            {
                ...
                MstRecognition Record = (from v in dbHrPayroll.MstRecognition where v.Id == ID select v).Single();
                dbHrPayroll.MstRecognition.DeleteOnSubmit(Record);
                dbHrPayroll.SubmitChanges();
