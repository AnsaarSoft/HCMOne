...
{
    class frm_EmpPosition:HRMSBaseForm
    {
        ...
        private MstPosition EmpPosition;
        ...
        private void InitiallizeForm()
        {
            try
            {
                oDBDataTable = oForm.DataSources.DataTables.Add("EmpPositions");
                ...
                clNo = oColumn;
                oColumn.DataBind.Bind("EmpPositions", "No");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_ID");
                clID = oColumn;
                oColumn.DataBind.Bind("EmpPositions", "I");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Name");
                clName = oColumn;
                oColumn.DataBind.Bind("EmpPositions", "N");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("cl_Desc");
                clDesc = oColumn;
                oColumn.DataBind.Bind("EmpPositions", "D");
                oColumn.TitleObject.Sortable = false;

                oColumn = oColumns.Item("clactive");
                clDesc = oColumn;
                oColumn.DataBind.Bind("EmpPositions", "Active");
                oColumn.TitleObject.Sortable = false;
            }
            catch (Exception ex)
            {
        ...
        private void GetDataFromDataSource()
        {
            try
            {
                ...
                var Data = from n in dbHrPayroll.MstPosition select n;
                    ...
                    case "Mat":
                        {
                            switch (pVal.ColUID)
                            {
                                case "cl_Name":
                                    ...
                                    if (Name.Trim().Equals("") && pVal.Row != oMat.RowCount)
                                    {
        ...
        private void AddRowToDataBase(string Name, string Desc, bool Active)
        {
            ...
            try
            {
                EmpPosition = new MstPosition();
                EmpPosition.Name = Name;
                EmpPosition.Description = Desc;
                EmpPosition.FlgActive = Active;
                EmpPosition.CreateDate = DateTime.Now;
                EmpPosition.UserId = oCompany.UserName;
                dbHrPayroll.MstPosition.InsertOnSubmit(EmpPosition);
                dbHrPayroll.SubmitChanges();
                try
                {
                    Query = string.Format("insert into OHPS (posID ,name ,descriptio ) values ({0},'{1}','{2}')", EmpPosition.Id, EmpPosition.Name, EmpPosition.Description);
                    RecSet.DoQuery(Query);
                }
                catch (Exception)
                {
                    Query = string.Format("update  OHPS set  name ='{1}' ,descriptio = '{2}'   where posID ={0} ", EmpPosition.Id, EmpPosition.Name, EmpPosition.Description);
                    RecSet.DoQuery(Query);
                    //oApplication.StatusBar.SetText("Code Already Exist in Table OHPS : Unable to add row in OHPS", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);
                }
            }
            catch (Exception ex)
            {
        ...
        private void UpdateExistingRecord(string Name, string Desc, int ID, bool Active)
        {
            ...
            try
            {
                //dbHrPayroll.ExecuteCommand("Update MstPosition set Name = {0},Description = {1},UpdateDate = {2} where ID = {3}", Name, Desc, DateTime.Now, ID);
                EmpPosition = (from v in dbHrPayroll.MstPosition where v.Id == ID select v).Single();
                EmpPosition.Name = Name;
                EmpPosition.Description = Desc;
                EmpPosition.FlgActive = Active;
                EmpPosition.UpdateDate = DateTime.Now;
                EmpPosition.UpdatedBy = oCompany.UserName;
                dbHrPayroll.SubmitChanges();
                Query = string.Format("Update \"OHPS\" set \"name\" = '{0}', \"descriptio\" = '{1}'where \"posID\" = {2}", Name, Desc, ID);
                RecSet.DoQuery(Query);
            }
            catch (Exception ex)
            {
